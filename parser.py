...
if __name__ == "__main__":
    ...
    for token in lexerInstance.tokens:
        if token[1] in keywords+delimiters:
            input_tape.append((token[1], token[2]))
        elif token[1] in bops:
            input_tape.append(('bop', token[2]))
        elif token[1] in uops:
            input_tape.append(('uop', token[2]))
        elif token[1].isnumeric():
            input_tape.append(('number', token[2]))
        elif token[1] in lexerInstance.symbolTable:
            input_tape.append(('id', token[2]))

    input_tape.append(('$', "end"))
    ...
    while len(input_tape) != 0:
        if stack[-1].value in terminals:
            ...
            if stack[-1].value == input_tape[0][0]:
                stack.pop()
                input_tape.pop(0)
            else:
                print('Error: non-matching terminals')
                errorFlag.append(input_tape[0][1])
                stack.pop()
        else:
            rule = parse_table_df[input_tape[0][0]][stack[-1].value]
        ...
        print(
            f'input tape: {[item[0] for item in input_tape]}\nstack: {stack}\n---')

    if (len(stack) == 0) and (len(input_tape) == 0) and len(errorFlag) == 0:
        print('parsing complete. string accepted.\n---')
    else:
        print("Errors encountered in line(s): ", end='')
        print(list(set(errorFlag)))
